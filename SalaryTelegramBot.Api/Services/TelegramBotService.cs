        ...
        if (text is "/start" or "/help")
        {
            ...
            if (text == "/help")
            {
                await output(
                    ...
                    🔐 Шифрование данных:
                    • Все суммы (начисления, выплаты, НДФЛ) зашифрованы вашим паролем
                    • Алгоритм: AES-256-GCM (используется банками и госсистемами)
                    • Пароль нигде не хранится — только вы знаете его
                    • Администратор бота НЕ может видеть ваши суммы
                    • Сессия пароля: 30 минут, затем нужно ввести заново
                    • ⚠️ Забыли пароль = данные потеряны навсегда

                    📌 Команды:
                    /status — общий долг и баланс
                    /history — таблица начислений и выплат
                    /recalc — пересчитать начисления по правилам
